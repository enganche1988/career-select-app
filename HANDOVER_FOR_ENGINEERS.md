...
**手順**:
1. `cd web && npm run dev`（ポートが3000埋まっていれば3001などに自動回避）
2. 別ターミナルで `ngrok http <port>`（例: `ngrok http 3000`）
3. ngrokの Forwarding に表示される https URL を共有

**運用ルール**:
- 共有するのは `/`, `/consultants`, `/consultants/[id]` のみ
