...
14. **暫定対応に触れる変更を行った場合、必ず関連ドキュメントも同時に更新すること。**
    - 暫定対応とは、`HANDOVER_FOR_ENGINEERS.md`に記載されている「暫定対応」セクションの内容を指す
    - 更新が必要なドキュメント:
      - `CURRENT_STATUS.md` - 現状の整理
      - `DEVELOPMENT_POLICY.md` - 開発方針
      - `HANDOVER_FOR_ENGINEERS.md` - エンジニア引き継ぎ用ドキュメント
    - 例: ダミーデータの追加/削除、buildスクリプトの変更、DB接続関連の変更など

15. **Vercelエラー対応は現フェーズでは原則行わない。**
    - 理由: build/型/環境差異で不安定になりやすく、検証フェーズでは沼るため
    - 共有方法: ローカル + ngrok による一時URL共有を正式手段とする
    - 詳細: `DEVELOPMENT_POLICY.md`の「共有・デプロイ方針」を参照
